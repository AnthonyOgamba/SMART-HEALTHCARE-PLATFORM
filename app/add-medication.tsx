...
export default function AddMedicationScreen() {
  ...
  return <ScreenContainer contentContainerStyle={styles.content}>
    <View style={styles.header}><Pressable accessibilityLabel="Go back" hitSlop={10} onPress={()=>router.back()}><MaterialIcons name="arrow-back" size={24} color={theme.primary}/></Pressable><Text style={styles.headerTitle}>{editing?'Edit Medication':'Add Medication'}</Text></View>
    ...
    <View style={styles.card}><Text style={styles.section}>Reminder Times</Text><TimePickerField label="Add reminder time" value="" onChange={addTime}/>
      <View style={styles.pills}>{times.map((time)=><View key={time} style={styles.pill}><Text style={styles.pillText}>{time}</Text><Pressable onPress={()=>setTimes(times.filter((x)=>x!==time))}><MaterialIcons name="close" size={16} color={theme.primary}/></Pressable></View>)}</View>
...
function Field({label,value,onChange,placeholder,multiline=false}:{label:string;value:string;onChange:(v:string)=>void;placeholder:string;multiline?:boolean}){const theme=usePalette(),styles=useMemo(()=>makeStyles(theme),[theme]);return <View><Text style={styles.label}>{label}</Text><TextInput style={[styles.input,multiline&&styles.multiline]} value={value} onChangeText={onChange} placeholder={placeholder} placeholderTextColor={theme.placeholder} multiline={multiline}/></View>;}
const makeStyles=(t:ThemePalette)=>StyleSheet.create({content:{padding:18,paddingBottom:80,gap:16,backgroundColor:t.screenBg},header:{flexDirection:'row',alignItems:'center',gap:12},headerTitle:{...PageTypography.title,color:t.accent},card:{backgroundColor:t.cardBg,borderWidth:1,borderColor:t.cardBorder,borderRadius:16,padding:18,gap:16},label:{color:t.textPrimary,fontWeight:'600',marginBottom:5},input:{height:48,borderWidth:1,borderColor:t.inputBorder,borderRadius:9,paddingHorizontal:13,color:t.textPrimary,backgroundColor:t.cardBg},multiline:{height:88,paddingTop:12},section:{fontSize:18,fontWeight:'700',color:t.accent},flex:{flex:1},pills:{flexDirection:'row',flexWrap:'wrap',gap:8},pill:{flexDirection:'row',alignItems:'center',gap:8,backgroundColor:t.blueTint,borderRadius:999,paddingHorizontal:12,paddingVertical:7},pillText:{color:t.textPrimary},soundOptions:{gap:8},soundOption:{minHeight:44,flexDirection:'row',alignItems:'center',gap:10,borderWidth:1,borderColor:t.inputBorder,borderRadius:9,paddingHorizontal:12,backgroundColor:t.cardBg},soundOptionSelected:{borderColor:t.primary,backgroundColor:t.selectedBackground},soundLabel:{color:t.textPrimary,fontWeight:'500'},switchRow:{flexDirection:'row',alignItems:'center'},muted:{color:t.textSecondary,fontSize:12},error:{color:t.danger,fontWeight:'600'},save:{height:54,borderRadius:12,backgroundColor:t.primaryButtonBackground,alignItems:'center',justifyContent:'center'},saveText:{color:t.white,fontSize:17,fontWeight:'700'},disabled:{opacity:.55}});
