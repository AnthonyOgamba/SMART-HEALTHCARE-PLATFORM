...
export async function getCareSchedule(date:string){
  ...
  const items:CareScheduleItem[]=[...meds.items.map(item=>({id:`m-${item.log.id}`,kind:'medication' as const,startsAt:item.log.scheduledFor,title:item.medication.name,subtitle:item.medication.dose,status:item.log.status,resourceId:item.medication.id,actionId:item.log.id})),...appointments.map(item=>({id:`a-${item.id}`,kind:'appointment' as const,startsAt:item.startsAt,title:item.title,subtitle:[item.providerName,item.location].filter(Boolean).join(' • '),status:item.status,resourceId:item.id,actionId:item.id,attendanceConfirmedAt:item.attendanceConfirmedAt})),...activities.map(item=>({id:`x-${item.id}`,kind:'activity' as const,startsAt:item.startedAt,title:item.activityType,subtitle:item.durationMinutes===null?'Scheduled activity':`${item.durationMinutes} minutes`,status:item.status,resourceId:item.id,actionId:item.id,seriesId:item.seriesId,completedAt:item.completedAt})),...sleep.map(item=>({id:`s-${item.id}`,kind:'sleep' as const,startsAt:item.sleepStart,title:'Sleep',subtitle:`${Math.floor(sleepDurationMinutes(item.sleepStart,item.wakeTime)/60)}h ${sleepDurationMinutes(item.sleepStart,item.wakeTime)%60}m`,status:'recorded',resourceId:item.id}))];
